classdef CbcmBeam < Beam
    properties
        bcmBeams=BcmBeam.empty;
        segments;
    end
    
    methods
        function beam=CbcmBeam(id,nodes,joints,angle,lengthBeam,crossSection)
            ...
            for i=1:length(crossSection.segments)
                beam.bcmBeams(i)=BcmBeam(id+(i-1)*4,nodes,joints,angle,lengthBeam*crossSection.segments(i),crossSection);
                beam.bcmBeams(i).noOfData=round(crossSection.segments(i)*100);
            end
        ...
        function beam=updateBeam(beam,inputs,index)
            ...
            for i=1:beam.segments
                beam.bcmBeams(i)=beam.bcmBeams(i).updateBeam(inputs,index);
            end
        end
        
        function initialGuess=getInitialGuess(beam,id)
            %get initial guess for the beam
            initialGuess=beam.bcmBeams(id).getInitialGuess();
        end
        ...
        function x = getX(beam,outputMatrix,distance)
            ...
            elseif distance == 0
                ...
                x=0;
                for i=1:beam.segments
                    singleDistance=beam.bcmBeams(i).length0/beam.length0*100;
                    if currentDistance + singleDistance >= distance
                        modifier=(distance-currentDistance)/singleDistance;
                        x=x+modifier*beam.bcmBeams(i).getX(outputMatrix,100);
                        return;
                    else
                        x=x+beam.bcmBeams(i).getX(outputMatrix,100);
        ...
        function y = getY(beam,outputMatrix,distance)
            ...
            elseif distance == 0
                ...
                for i=1:beam.segments
                    ...
                    if currentDistance + singleDistance >= distance
                        modifier=(distance-currentDistance)/singleDistance;
                        y=y+modifier*beam.bcmBeams(i).getY(outputMatrix,100);
                        return;
                    else
                        y=y+beam.bcmBeams(i).getY(outputMatrix,100);
                        currentDistance=currentDistance+singleDistance;
                    end
                end
            end
        ...
        function gradientX = getGradientX(beam,outputMatrix,index,distance)
            ...
            elseif distance == 0
                ...
                for i=1:beam.segments
                    ...
                    if currentDistance + singleDistance >= distance
                        modifier=(distance-currentDistance)/singleDistance;
                        gradientX=gradientX+modifier*beam.bcmBeams(i).getGradientX(outputMatrix,index,100);
                        return;
                    else
                        gradientX=gradientX+beam.bcmBeams(i).getGradientX(outputMatrix,index,100);
                        currentDistance=currentDistance+singleDistance;
                    end
                end
            end
        ...
        function gradientY = getGradientY(beam,outputMatrix,index,distance)
            ...
            elseif distance == 0
                ...
                for i=1:beam.segments
                    ...
                    if currentDistance + singleDistance >= distance
                        modifier=(distance-currentDistance)/singleDistance;
                        gradientY=gradientY+modifier*beam.bcmBeams(i).getGradientY(outputMatrix,index,100);
                        return;
                    else
                        gradientY=gradientY+beam.bcmBeams(i).getGradientY(outputMatrix,index,100);
                        currentDistance=currentDistance+singleDistance;
                    end
                end
            end
        ...
        function hessianX = getHessianX(beam,outputMatrix,index,distance)
            ...
            elseif distance == 0
                ...
                for i=1:beam.segments
                    ...
                    if currentDistance + singleDistance >= distance
                        modifier=(distance-currentDistance)/singleDistance;
                        hessianX=hessianX+modifier*beam.bcmBeams(i).getHessianX(outputMatrix,index,100);
                        return;
                    else
                        hessianX=hessianX+beam.bcmBeams(i).getHessianX(outputMatrix,index,100);
                        currentDistance=currentDistance+singleDistance;
                    end
                end
            end
        ...
        function hessianY = getHessianY(beam,outputMatrix,index,distance)
            ...
            elseif distance == 0
                ...
                for i=1:beam.segments
                    ...
                    if currentDistance + singleDistance >= distance
                        modifier=(distance-currentDistance)/singleDistance;
                        hessianY=hessianY+modifier*beam.bcmBeams(i).getHessianY(outputMatrix,index,100);
                        return;
                    else
                        hessianY=hessianY+beam.bcmBeams(i).getHessianY(outputMatrix,index,100);
                        currentDistance=currentDistance+singleDistance;
                    end
                end
            end
        ...
        function energy = getEnergy(beam,outputMatrix,index,workspace)
            energy=0;
            for i=1:beam.segments
                energy=energy+beam.bcmBeams(i).getEnergy(outputMatrix,index,workspace);
            end
        end
        ...
        function gradient=getGradient(beam,outputMatrix,index,workspace)
            ...
            for i=1:beam.segments
                gradient=gradient+beam.bcmBeams(i).getGradient(outputMatrix,index,workspace);
            end
        end
        ...
        function hessian=getHessian(beam,outputMatrix,index,workspace)
            ...
            for i=1:beam.segments
                hessian=hessian+beam.bcmBeams(i).getHessian(outputMatrix,index,workspace);
            end
        end
        ...
        function beam=updateInternalID(beam,modifier)
            %adjust bcm ids
            for i=1:beam.segments
                beam.bcmBeams(i).id=beam.bcmBeams(i).id+modifier;
            end
        end
        ...
        function [angle,index]=getAngle(beam,distance)
            ...
            for i=1:beam.segments
                singleDistance=beam.bcmBeams(i).length0/beam.length0*100;
                if currentDistance + singleDistance >= distance
                    angle=beam.bcmBeams(i).theta;
                    index=beam.bcmBeams(i).index(4);
                    return;
                end
                currentDistance=currentDistance+singleDistance;
            end
            %numerical error check
            angle=beam.bcmBeams(end).theta;
            index=beam.bcmBeams(end).index(4);
        end
        ...
        function force=getForce(beam,workspace)
            ...
            deltaX=beam.bcmBeams(end).deltaX*workspace.lengthFactor();
            deltaY=beam.bcmBeams(end).deltaY*workspace.lengthFactor();
            deltaTheta=beam.bcmBeams(end).getDeltaTheta();
            EI=beam.bcmBeams(end).crossSection.E*workspace.EFactor()*beam.bcmBeams(end).crossSection.getI()*workspace.lengthFactor()^4;
            L=beam.bcmBeams(end).length0*workspace.lengthFactor();
            t=beam.bcmBeams(end).crossSection.thickness*workspace.lengthFactor();
            pLoad=beam.bcmBeams(end).p(deltaX,deltaY,deltaTheta,t,L);
            fmLoad=beam.bcmBeams(end).fm(deltaY,deltaTheta,pLoad,L); 
            %to global coordinate
            forces=[pLoad*EI/L^2;...
                fmLoad(1)*EI/L^2;....
                fmLoad(2)*EI/L];
            force(1,1)=(forces(1)*cos(beam.bcmBeams(end).masterAngle+beam.bcmBeams(end).theta0)-forces(2)*sin(beam.bcmBeams(end).masterAngle+beam.bcmBeams(end).theta0))/workspace.forceFactor();
            force(1,2)=(forces(1)*sin(beam.bcmBeams(end).masterAngle+beam.bcmBeams(end).theta0)+forces(2)*cos(beam.bcmBeams(end).masterAngle+beam.bcmBeams(end).theta0))/workspace.forceFactor();
        ...
        function beam=getStressShape(beam,workspace)
            ...
            beam.yValues=[];
            for i=1:beam.segments
                [x,y,stress]=beam.bcmBeams(i).getShape(workspace);
        ...
        function beam=drawLink(beam,currentAxis,nodes,limit,mode,parent)
            ...
            deltaY=node1.y;
            for i=1:beam.segments
                [xx,yy,stress]=beam.bcmBeams(i).getShape(parent.getWorkspace());
                x{i}=xx+deltaX;
                y{i}=yy+deltaY;
            ...
            if ~isempty(beam.crossSection)
                lineWidthBeam=max(1,beam.crossSection.thickness);
        ...
        function beam=drawLinkNoGUI(beam,currentAxis,nodes,limit,workspace)
            ...
            deltaY=node1.y;
            for i=1:beam.segments
                [xx,yy,stress]=beam.bcmBeams(i).getShape(workspace);
                x{i}=xx+deltaX;
                y{i}=yy+deltaY;
                bendingStress{i}=stress;
                deltaX=deltaX+xx(end);
