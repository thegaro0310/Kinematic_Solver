function [newState,exitflag,energy]=bistableAnalysis(allBeams,torsionSprings,kinematicEq,input,additionalDof,lengthFactor,forceFactor,EFactor)
[~,~,initialGuess] = findInputMatrix(allBeams,zeros(allBeams(end).id+allBeams(end).degreesOfFreedom+3,1),input,additionalDof);
fMincon=@(x)fMinConFunc( x,allBeams,torsionSprings,input,additionalDof,lengthFactor,forceFactor,EFactor);
gMinCon=@(x)fMinConCostraint( x,allBeams,input,kinematicEq,additionalDof);
opt=optimoptions(@fmincon,'Display','none','Algorithm','interior-point','MaxFunEvals',50000,'MaxIter',1000,'ConstraintTolerance',1e-8,'FunctionTolerance',1e-10,...
    'OptimalityTolerance',1e-10,'SpecifyConstraintGradient',true,'SpecifyObjectiveGradient',true...
    ,'HessianFcn',@(x,lambda)hessianfcn(x,lambda,allBeams,torsionSprings,input,kinematicEq,additionalDof,lengthFactor,forceFactor,EFactor)...
...
function [f,g] = fMinConFunc(x,allBeams,torsionSprings,input,additionalDof,lengthFactor,forceFactor,EFactor)
...
f=(energy)/(lengthFactor()*forceFactor());
...
function Hout = hessianfcn(x,lambda,allBeams,torsionSprings,input,kinematicEq,additionalDof,lengthFactor,forceFactor,EFactor)
...
switch beam.class
    ...
    case {'CbcmBeam','MlinearBeam'}
        ...
        for i=1:length(beam.childIds)
            singleDistance=beam.beam.childLengths(i)/beam.length0*100;
            if currentDistance + singleDistance >= distance
